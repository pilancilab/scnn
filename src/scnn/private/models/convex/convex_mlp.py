...
class ConvexMLP(Model):
    ...
    def _objective(
        ...
        **kwargs,
        ...
        # )
        return huber_loss(self._forward(X, w, D), y, 1.0) / (
                self._scaling(y, scaling)
        )

    def expanded_data_matrix(self, X: lab.Tensor, D: lab.Tensor) -> lab.Tensor:
    ...
    def _gradient_huber(self, w, X, y, signs, delta=1.0):
        pred = self._data_mvp(w, X, signs)
        error = y - pred
        mask_small = (lab.abs(error) <= delta).flatten()

        w_flat = w.reshape(-1)
        grad = lab.zeros_like(w_flat)

        X_expanded = self.expanded_data_matrix(X, signs)
        grad -= (X_expanded[mask_small, :].T @ error[mask_small]).flatten()

        grad -= (delta * X_expanded[~mask_small, :].T @ lab.sign(error[~mask_small])).flatten()

        return grad.reshape(*w.shape)

    def _gradient_huber_fast(self, w, X, y, signs, delta=1.0):
    ...
    def _grad(
        ...
        **kwargs,
        ...
        #     y, scaling
        # )
        grad = self._gradient_huber_fast(w, X, y, self._signs(X, D)) / self._scaling(y, scaling)
...
class HuberMLP(ConvexMLP):
    ...
    def _objective(
        ...
        **kwargs,
        ...
        return huber_loss(self._forward(X, w, D), y, self._huber_delta) / (
            2 * self._scaling(y, scaling)
        )
